...
@php 
    ...
    $title_refinancing = explode(" ",$modality_name);
    $title_refinancing = $title_refinancing[0] == "Refinanciamiento" ? '':"PRÉSTAMO ";
@endphp
...
@if(($modality_name == 'Refinanciamiento de Préstamo a Corto Plazo Sector Activo' || 
    $modality_name == 'Refinanciamiento de Préstamo a Corto Plazo Sector Pasivo SENASIR' || 
    $modality_name == 'Refinanciamiento de Préstamo a Corto Plazo sector Pasivo AFP' || 
    $modality_name == 'Refinanciamiento de Préstamo a Corto Plazo en Disponibilidad' || 
    $modality_name == 'Refinanciamiento de Préstamo a Corto Plazo sector Pasivo Gestora Pública'))

<div class="block text-justify">
    <div>
        Conste en el presente contrato de {{ ucfirst($title) }}, que al solo reconocimiento de firmas y rúbricas será elevado a 
    ...
    <div>
        @if($loan->data_loan)
            ...
            (<span class="uppercase">{{ Util::money_format($loan->data_loan->amount_approved, true) }} Bolivianos) </span>, con garantía de 
            rentas y/o haberes y garantía personal segun corresponda.
            <div>
            ...
            {{ Util::money_format($loan->refinancing_balance, true) }} Bolivianos)</span>, para lo cual el PRESTATARIO reconoce de manera 
            expresa el saldo anterior de la deuda correspondiente al préstamo contraído con anterioridad, que asciende a la suma de 
            Bs.{{Util::money_format( $loan->balance_parent_refi())}} (<span class="uppercase">{{ Util::money_format($loan->balance_parent_refi(), true) }} 
            Bolivianos)</span>, montos que hacen un total efectivo de Bs.{{ Util::money_format($loan->amount_approved) }} 
            (<span class="uppercase">{{ Util::money_format($loan->amount_approved, true) }} Bolivianos)</span>, que representa la 
            nueva obligación contraída sujeta a cumplimiento, en función a la operación de refinanciamiento.
            </div>
        @else
            <div>
            ...
            (<span class="uppercase">{{ Util::money_format($parent_loan->amount_approved, true) }} Bolivianos)</span>, con garantía de rentas y/o 
            haberes y garantía personal segun corresponda.
            </div>
            <div>
            <b>TERCERA.- (DEL OBJETO):</b>  El objeto del presente contrato es el refinanciamiento del préstamo de dinero que la MUSERPOL otorga 
            al PRESTATARIO conforme a calificación, previa evaluación y autorización, de conformidad a los niveles de aprobación respectivos en la 
            suma de Bs.{{ Util::money_format($loan->refinancing_balance) }} (<span class="uppercase">{{ Util::money_format($loan->refinancing_balance, true) }} 
            Bolivianos) </span>, para lo cual el PRESTATARIO reconoce de manera expresa el saldo anterior de la deuda correspondiente al préstamo contraído 
            con anterioridad, que asciende a la suma de Bs.{{ Util::money_format($loan->balance_parent_refi())}} (<span class="uppercase">
            {{ Util::money_format($loan->balance_parent_refi(), true) }} Bolivianos)</span>, montos que hacen un total efectivo de 
            Bs.{{ Util::money_format($loan->amount_approved) }} (<span class="uppercase">{{ Util::money_format($loan->amount_approved, true) }} 
            Bolivianos)</span>, que representa la nueva obligación contraída sujeta a cumplimiento, en función a la operación de refinanciamiento.
            </div>
        @endif
    </div>
    <div>
        <b>CUARTA.- (DEL INTERÉS):</b> El préstamo objeto del presente contrato, devengará un interés ordinario del {{ round($loan->interest->annual_interest) }}% 
        anual sobre saldo deudor, el mismo que se recargará con el interés penal en caso de mora de una o más amortizaciones.
    </div>
    <div>
        <b>QUINTA.- (DEL PLAZO Y LA CUOTA DE AMORTIZACIÓN):</b> El plazo fijo e improrrogable para el cumplimiento de la obligación contraída por el 
        PRESTATARIO en virtud al préstamo otorgado es de {{ $loan->loan_term }} {{ $loan->loan_term == 1 ? 'mes' : 'meses' }} computables a partir de la fecha de desembolso. La cuota de amortización 
        mensual es de Bs.{{ Util::money_format($loan->estimated_quota) }} (<span class="uppercase">{{ Util::money_format($loan->estimated_quota, true) }} 
        Bolivianos)</span>.
    </div>
    <div>
        Los intereses generados entre la fecha del desembolso del préstamo y la fecha del primer pago serán cobrados con la primera cuota; conforme 
        los establece el Reglamento de Préstamos.
    </div>
    <div>
        <b>SEXTA.- (DEL DESEMBOLSO):</b> El desembolso del préstamo de dinero en la moneda pactada se acredita mediante comprobante escrito en el 
        que conste el abono efectuado a favor del  PRESTATARIO, a través de una cuenta bancaria señalada por el mismo, reconociendo ambas partes que 
        al amparo de este procedimiento se cumple satisfactoriamente la exigencia contenida en el artículo 1331 del Código de Comercio.
    </div>
    <div>
        @if($modality_name == 'Refinanciamiento de Préstamo a Corto Plazo sector Pasivo AFP' || 
            $modality_name == 'Refinanciamiento de Préstamo a Corto Plazo sector Pasivo Gestora Pública')
        <b>SEPTIMA.- (DE LA FORMA DE PAGO Y OTRAS CONTINGENCIAS):</b> Para el cumplimiento estricto de la obligación (capital e intereses) el 
        PRESTATARIO, se obliga a cumplir con la cuota de amortización en forma mensual mediante pago directo en la oficina central de la MUSERPOL 
        de la ciudad de La Paz o efectuar el depósito en la cuenta fiscal de la MUSERPOL. Caso contrario el PRESTATARIO se hará pasible al recargo 
        correspondiente a los intereses que se generen al día de pago por la deuda contraída. Consecuentemente se procederá al descuento del 
        garante personal incluido los intereses penales una vez transcurrido las dos cuotas impagas, sin necesidad de previo aviso.
        @else
            @php
                if($modality_name == 'Refinanciamiento de Préstamo a Corto Plazo Sector Activo' || 
                    $modality_name == 'Refinanciamiento de Préstamo a Corto Plazo en Disponibilidad'){
                ...
                if($modality_name =='Refinanciamiento de Préstamo a Corto Plazo Sector Pasivo SENASIR'){
                    $septima = 'Servicio Nacional del Sistema de Reparto SENASIR';
                    $type_rent='las rentas';
                }
            @endphp
            <b>SEPTIMA.- (DE LA FORMA DE PAGO Y OTRAS CONTINGENCIAS):</b> Para el cumplimiento estricto de la obligación (capital e intereses) el 
            PRESTATARIO, autoriza expresamente a la MUSERPOL practicar los descuentos respectivos de {{$type_rent}} que percibe en forma mensual 
            a través del {{ $septima }}, conforme al Reglamento de Prestamos.
            <div>
    ...
    <div>
        <b>OCTAVA.- (DERECHOS DEL PRESTATARIO):</b> Conforme al Artículo 10 del Reglamento de Préstamos las partes reconocen expresamente como derechos 
        del PRESTATARIO, lo siguiente:
    </div>
    <div>
        <ol type="a" style="margin:0;">
            <li>
                Recibir buena atención, trato equitativo y digno por parte de los funcionarios de la MUSERPOL sin discriminación de ninguna naturaleza, 
                asimismo recibir información y orientación precisa, comprensible, oportuna y accesible con relación a requisitos, características y 
                condiciones del préstamo.
            </li>
            <li>
                A la confidencialidad, información detallada y precisa concerniente a los préstamos bajo su titularidad en el marco estricto 
    ...
    <div>
        <b>NOVENA.- (OBLIGACIONES DEL PRESTATARIO):</b> Conforme al Artículo 11 del Reglamento de Préstamos, las partes reconocen expresamente 
    ...
    <div>
        @if($modality_name == 'Refinanciamiento de Préstamo a Corto Plazo sector Pasivo AFP' || 
            $modality_name == 'Refinanciamiento de Préstamo a Corto Plazo sector Pasivo Gestora Pública')
        <b>DECIMA.- (DE LA GARANTÍA):</b>El PRESTATARIO y GARANTE, garantizan el pago de lo adeudado con la generalidad de sus bienes, derechos y acciones 
        habidos y por haber presentes y futuros conforme lo determina el Art. 1335 del Código Civil, ademas el PRESTATARIO con los beneficios otorgados por la MUSERPOL.
        Asimismo, se constituye como garante personal, solidario, mancomunado e indivisible:
            @if (count($guarantors) == 1)
                ...
                posterior, sin perjuicio de que éstos puedan iniciar las acciones legales correspondientes en contra del PRESTATARIO.
                </span>
            @endif
        @elseif($modality_name == 'Refinanciamiento de Préstamo a Corto Plazo Sector Activo' || 
                $modality_name == 'Refinanciamiento de Préstamo a Corto Plazo en Disponibilidad')
                <b>DECIMA.- (DE LA GARANTÍA):</b> El PRESTATARIO, garantiza el pago de lo adeudado con la generalidad de sus bienes, 
                derechos y acciones habidos y por haber, presentes y futuros conforme determina el Art. 1335 del Código Civil, 
                asimismo con los beneﬁcios otorgados por la MUSERPOL.
        @elseif($modality_name =='Refinanciamiento de Préstamo a Corto Plazo Sector Pasivo SENASIR')
                <b>DECIMA.- (DE LA GARANTÍA):</b> El PRESTATARIO, garantiza el pago de lo adeudado con la generalidad de sus bienes, 
                derechos y acciones habidos y por haber, presentes y futuros conforme determina el Art. 1335 del Código Civil, y asi 
    ...
    <div>
        @if($modality_name == 'Refinanciamiento de Préstamo a Corto Plazo sector Pasivo AFP' || 
            $modality_name == 'Refinanciamiento de Préstamo a Corto Plazo sector Pasivo Gestora Pública')
            <div>
                <b>DÉCIMA PRIMERA.- (CONTINGENCIAS POR FALLECIMIENTO):</b>El PRESTATARIO en caso de fallecimiento acepta amortizar para 
                el cumplimiento efectivo de la presente obligación con el beneficio del Complemento Económico en caso de corresponderle; 
                por cuanto la liquidación de dicho beneficio pasarán a cubrir el monto total de la obligación que resulte adeudada, más 
                los intereses devengados a la fecha, cobrados a los derechohabientes, previas las formalidades de ley. 
            </div>
        @else
            @if($modality_name == 'Refinanciamiento de Préstamo a Corto Plazo Sector Activo' || 
                $modality_name == 'Refinanciamiento de Préstamo a Corto Plazo en Disponibilidad')
            <div>
                ...
                retiro voluntario o retiro forzoso garantizan el cumplimiento efectivo de la presente obligación con la totalidad 
            ...
            @elseif($modality_name =='Refinanciamiento de Préstamo a Corto Plazo Sector Pasivo SENASIR')
            <div>
                <b>DÉCIMA PRIMERA.- (CONTINGENCIAS POR FALLECIMIENTO):</b>El PRESTATARIO en caso de fallecimiento acepta a amortizar 
                el cumplimiento efectivo de la presente obligación con el beneficio del Complemento Económico en caso de corresponderle; 
                por cuanto la liquidación de dicho beneficio pasara a cubrir el monto total de la obligación que resulte adeudada, más 
                los intereses devengados a la fecha, cobrados a los derechohabientes, previas las formalidades de ley. 
            </div>
            @endif
        @endif
    ...
    <div>
        <b>DÉCIMA SEGUNDA.- (DE LA MORA):</b> El PRESTATARIO se constituirá en mora automática sin intimación o requerimiento alguno, 
        de acuerdo a lo establecido por el artículo 341, Núm. 1) del Código Civil, al incumplimiento del pago de cualquier amortización 
        de capital e intereses, sin necesidad de intimación o requerimiento alguno, o acto equivalente por parte de la MUSERPOL.
    </div>
    <div>
        Además del interés acordado contractualmente, el préstamo generará en caso de mora un interés moratorio anual del 6% sobre 
        saldos de capital de las cuotas impagas, aún cuando fuere exigible todo el capital del préstamo.
    </div>
    <div>
        <b>DÉCIMA TERCERA.- (DE LOS EFECTOS DEL INCUMPLIMIENTO Y DE LA ACCIÓN EJECUTIVA):</b> El incumplimiento de pago mensual por 
    ...
    @if($modality_name == 'Refinanciamiento de Préstamo a Corto Plazo sector Pasivo AFP')
        <div>
            ...
            domicilio especial del PRESTATARIO y GARANTE el señalado en la cláusula primera y decima  de conformidad al artículo 29 
            parágrafo II del Código Civil, donde se efectuarán las citaciones y notificaciones judiciales o cualquier otra comunicación, 
            con plena validez legal y sin lugar a posterior observación o recurso alguno.
        </div>
    @else
        <div>
            <b>DÉCIMA CUARTA.- (DOMICILIO ESPECIAL):</b> Para efectos legales, incluida la acción judicial u otra, se tendrá como 
            domicilio especial del PRESTATARIO el señalado en la cláusula primera de conformidad al artículo 29 parágrafo II del 
            Código Civil, donde se efectuarán las citaciones y notificaciones judiciales o cualquier otra comunicación, con plena 
            validez legal y sin lugar a posterior observación o recurso alguno.
        </div>
    @endif
    <div>
        <b>DÉCIMA QUINTA.- (DE LA CONFORMIDAD Y ACEPTACIÓN):</b> Por una parte en calidad de ACREEDOR la Mutual de Servicios al 
        Policía (MUSERPOL), representada por su {{ $employees[0]['position'] }} {{ $employees[0]['name'] }} y su 
        ...
        @if (count($lenders) == 1)
            <span>
                ...
                cumplimiento. En señal de lo cual suscribimos el presente contrato de refinancimiento de préstamo de dinero a corto plazo 
                en manifestación de nuestra libre y espontánea voluntad y sin que medie vicio de consentimiento alguno.
            </span>
        @endif
    </div><br><br>
...
<div class="block text-justify">
    <div>
        Conste en el presente contrato de préstamo a {{ ucfirst($title) }}, que al solo reconocimiento de firmas y 
    ...
    <div>
        ...
        Policía (MUSERPOL) otorga al PRESTATARIO conforme a niveles de aprobación respectivos, en la suma de 
        Bs.{{ Util::money_format($loan->amount_approved) }} (<span class="uppercase">{{Util::money_format($loan->amount_approved, true)}} 
        Bolivianos).</span>
    </div>
    <div>
    ...
    <div>
        ...
        obligación contraída por el PRESTATARIO en virtud al préstamo otorgado es de {{ $loan->loan_term }} {{ $loan->loan_term == 1 ? 'mes' : 'meses' }} computables 
        a partir de la fecha de desembolso. La cuota de amortización mensual es de Bs.{{ Util::money_format($loan->estimated_quota) }} (<span class="uppercase">
    ...
    <div>
        ...
        reconociendo ambas partes que al amparo de este procedimiento se cumple satisfactoriamente la exigencia contenida en el artículo 
    ...
    <div>
        @if($modality_name == 'Corto Plazo Sector Pasivo AFP' || 
            ...
            hará pasible al recargo correspondiente a los intereses que se generen al día de pago por la deuda contraída. Consecuentemente 
            se procederá al descuento al garante personal incluido los intereses penales una vez transcurrido las dos cuotas impagas, 
        ...
        @else
            ...
            de {{$type_rent}} que percibe en forma mensual a través del {{ $sexta }} conforme al Reglamento de Préstamos.
            <div>
                Si por cualquier motivo la MUSERPOL estuviera imposibilitada de realizar el descuento por el medio señalado, 
                el PRESTATARIO se obliga a cumplir con la cuota de amortización mediante pago directo en la Oficina Central de 
                la MUSERPOL de la ciudad de La Paz o efectuar el depósito en la cuenta fiscal de la MUSERPOL. Caso contrario 
    ...
    <div>
        <b>SÉPTIMA.- (DERECHOS DEL PRESTATARIO):</b> Conforme al Artículo 10 del Reglamento de Préstamos las partes reconocen 
        expresamente como derechos del PRESTATARIO, lo siguiente:
    </div>
    <div>
        <ol type="a" style="margin:0;">
            <li>
    ...
    <div>
        <b>OCTAVA.- (OBLIGACIONES DEL PRESTATARIO):</b> Conforme al Artículo 11 del Reglamento de Préstamos, las partes 
        reconocen expresamente como obligaciones del PRESTATARIO, lo siguiente:
    </div>
    <div>
        <ol type="a" style="margin:0;">
            <li>Proporcionar información y documentación veraz y legítima para la correcta tramitación del préstamo.</li>
    ...
    <div>
        @if($modality_name == 'Corto Plazo Sector Pasivo AFP' || 
            ...
            @if (count($guarantors) == 1)
                ...
                MUSERPOL de toda responsabilidad o reclamo posterior, sin perjuicio de que este pueda iniciar las acciones legales correspondientes 
                en contra del PRESTATARIO.</div>
                </span>
            @endif
    ...
    @if($modality_name == 'Corto Plazo Sector Pasivo AFP' || 
        ...
        <div>
            ...
            cuanto la liquidación de dicho beneficio pasará a cubrir el monto total de la obligación que resulte adeudada, más los intereses 
    ...
    <div>
        <b>DÉCIMA PRIMERA.- (DE LA MORA):</b> El PRESTATARIO se constituirá en mora automática sin intimación o requerimiento 
    ...
    <div>
        Además del interés acordado contractualmente, el préstamo generará en caso de mora un interés moratorio anual del 6% 
        sobre saldos de capital de las cuotas impagas, aún cuando fuere exigible todo el capital del préstamo.
    </div>
    <div>
        <b>DÉCIMA SEGUNDA.- (DE LOS EFECTOS DEL INCUMPLIMIENTO Y DE LA ACCIÓN EJECUTIVA):</b> El incumplimiento de pago mensual 
        por parte del PRESTATARIO dará lugar a que la totalidad de la obligación, incluidos los intereses moratorios, se determinen 
        líquidos, exigibles y de plazo vencido quedando la MUSERPOL facultada de iniciar las acciones legales correspondientes al 
        amparo de los artículos 519 y 1465 del Código Civil así como de los artículos 378 y 379, Núm. 2) del Código Procesal Civil, 
        que otorgan a este documento la calidad de Título Ejecutivo, demandando el pago de la totalidad de la obligación contraída, 
        más la cancelación de intereses convencionales y penales, incluyendo los relacionados y emergentes de la cobranza judicial, 
    ...
    <div>
        En caso de incumplimiento de los pagos mensuales estipulados en el presente contrato que generen mora de la obligación, el 
        PRESTATARIO no tendrá derecho a acceder a otro crédito, hasta la cancelación total de la deuda.
    </div>
    @if($modality_name == 'Corto Plazo Sector Pasivo AFP')
        <div>
            <b>DÉCIMA TERCERA.- (DOMICILIO ESPECIAL):</b> Para efectos legales, incluida la acción judicial u otra, se tendrá como 
            domicilio especial del PRESTATARIO y GARANTE el señalado en la cláusula primera y novena de conformidad al artículo 29 
            parágrafo II del Código Civil, donde se efectuarán las citaciones y notificaciones judiciales o cualquier otra comunicación, 
            con plena validez legal y sin lugar a posterior observación o recurso alguno.
        </div>
    @else
    ...
    @endif
    <div>
        <b>DÉCIMA CUARTA.- (DE LA CONFORMIDAD Y ACEPTACIÓN):</b> Por una parte en calidad de ACREEDOR la Mutual de Servicios al 
        Policía (MUSERPOL), representada por su {{ $employees[0]['position'] }} {{ $employees[0]['name'] }} y su 
        {{ $employees[1]['position'] }} {{ $employees[1]['name'] }} y por otra parte en calidad de
        @if (count($lenders) == 1)
            <span>
                PRESTATARIO {{ $lender->gender == 'M' ? '' : 'a' }} {{ $lender->gender == 'M' ? 'el Sr.' : 'la Sra' }} 
                {{ $lender->full_name }} de generales ya señaladas,
                @if($modality_name == 'Corto Plazo Sector Pasivo AFP' || 
                    $modality_name == 'Corto Plazo Sector Pasivo Gestora Pública')
                    @if (count($guarantors) == 1)
                        @php 
                ...
                En señal de lo cual suscribimos el presente contrato de préstamo de dinero a corto plazo en manifestación de nuestra 
                libre y espontánea voluntad y sin que medie vicio de consentimiento alguno.
            </span>
        @endif
    </div><br><br>
    <div class="text-center">
