...
<div class="block text-justify">
    ...
    <div>
        <b>PRIMERA.- (DE LAS PARTES):</b> Intervienen en el presente contrato, por una parte la Mutual de Servisios al Policía (MUSERPOL), representada legalmente por su {{ $employees[0]['position'] }} {{ $employees[0]['name'] }} con C.I. {{ $employees[0]['identity_card'] }} y su {{ $employees[1]['position'] }} {{ $employees[1]['name'] }} con C.I. {{ $employees[1]['identity_card'] }}, que para fines de este contrato en adelante se denominará MUSERPOL o ACREEDOR con domicilio en la Z. Sopocachi, Av. 6 de Agosto Nº 2354 y por otra parte
    ...
    <?php $modality = $loan->modality;
        ...
        <?php }
            ...
            ?>
        <b>SEPTIMA.- (DE LA FORMA DE PAGO Y OTRAS CONTINGENCIAS):</b> Para el cumplimiento estricto de la obligación (capital e intereses) el PRESTATARIO, autoriza expresamente a MUSERPOL practicar los descuentos respectivos {{$is_active? 'de los haberes':'de las rentas'}} que percibe en forma mensual a través del {{ $septima }}.
        <div>
        ...
        <div>
            Asimismo, el PRESTATARIO se compromete a hacer conocer oportunamente a MUSERPOL sobre la omisión del descuento mensual que se hubiera dado a efectos de solicitar al {{ $septima }} regularice este descuento sin perjuicio que realice el depósito directo del mes omitido de acuerdo a lo estipulado en el párrafo precedente. Caso contrario se procedera al descuento a los garantes personales pasado los 30 dias de incumplimiento sin necesidad de previo aviso.
        </div>
        <?php }?>
    </div>
    <div>
    ...
    <div>
     <?php $modality = $loan->modality;
        ...
        <br> Se {{ count($guarantors) > 1 ? 'constituyen como garantes personales, solidarios, mancomunados, e indivisibles' : 'constituye como garante personal, solidario, mancomunado e indivisible' }}:
        <?php } else{
        if($modality->name == 'Refinanciamiento de Préstamo a Largo Plazo Sector Pasivo AFP'){ ?>
        <b>DECIMA.- (DE LA GARANTÍA):</b>El PRESTATARIO y {{ count($guarantors)>1 ? 'GARANTES':'GARANTE' }}, garantizan el pago de lo adeudado con la generalidad de sus bienes, derechos y acciones habidos y por haber, presentes y futuros conforme lo determina el artículo 1335 del Código Civil, asimismo el PRESTATARIO, con los beneficios otorgados por la MUSERPOL.
    ...
    <div>
        <b>DÉCIMA CUARTA.- (DOMICILIO ESPECIAL):</b> Para efectos legales, incluida la acción judicial u otra, se tendrá como domicilio especial del PRESTATARIO y GARANTES el señalado en la cláusula primera y decima de conformidad al artículo 29 parágrafo II del Código Civil, donde se efectuarán las citaciones y notificaciones judiciales o cualquier otra comunicación, con plena validez legal y sin lugar a posterior observación o recurso alguno.
    </div>
    <div>
