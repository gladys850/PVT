...
<div class="block text-justify">
    ...
    <?php $modality = $loan->modality;
        ...
        <?php }
            ...
            <div>
                Asimismo, el PRESTATARIO se compromete a hacer conocer oportunamente a la MUSERPOL sobre la omisión del descuento mensual que se hubiera dado a efectos de solicitar al {{ $sexta }} regularice este descuento, sin perjuicio que realice el depósito directo del mes omitido, de acuerdo a lo estipulado en el párrafo precedente. Caso contrario se procedera al descuento {{ count($guarantors) > 1 ? 'de los garantes personales' : 'del garante personal' }} pasados los 30 dias de incumplimiento, sin necesidad de previo aviso.
            </div>
        <?php }
    } ?>
    </div>
    ...
    <div>
     <?php $modality = $loan->modality;
        ...
        <?php } else{
            if($modality->name == 'Largo Plazo con Garantía Personal Sector Pasivo AFP'){ ?>
            <b>NOVENA.- (DE LA GARANTÍA):</b> El PRESTATARIO y GARANTES, garantizan el pago de lo adeudado con todos sus bienes, derechos y acciones habidos y por haber, presentes y futuros conforme lo determina el artículo 1335 del Código Civil, además el PRESTATARIO con los beneficios otorgados por la MUSERPOL
    ...
    <?php $modality = $loan->modality;
        if($modality->name == 'Largo Plazo con Garantía Personal Sector Activo' || $modality->name == 'Largo Plazo con un Solo Garante Sector Activo' || $modality->name == 'Largo Plazo con Garantía Personal en Comisión'){ ?>
          <div>
          {{ count($guarantors) > 1 ? 'Los GARANTES están facultados' : 'El GARANTE esta facultado' }} a realizar el trámite de recuperación de los montos que se le hubieran sido descontados en función a la obligación objeto del presente contrato pudiendo recaer sobre él Beneficio de Fondo de Retiro Policial Solidario otorgados por la MUSERPOL y reconocidos al PRESTATARIO, conforme al artículo 65 del Reglamento de Préstamos.
        </div>
        <?php } ?>
    <div>
        En caso de incumplimiento de los pagos mensuales estipulados en el presente contrato que generen mora de la obligación, el PRESTATARIO no tendrá derecho a acceder a otro crédito, hasta la cancelación total de la deuda.
    ...
    <div>
        <b>DÉCIMA CUARTA.- (DE LA CONFORMIDAD Y ACEPTACIÓN):</b> Por una parte en calidad de ACREEDOR la MUSERPOL, representada por su {{ $employees[0]['position'] }} {{ $employees[0]['name'] }} y su {{ $employees[1]['position'] }} {{ $employees[1]['name'] }} y por otra parte en calidad de
        @if (count($lenders) == 1)
