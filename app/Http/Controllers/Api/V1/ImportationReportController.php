        ...
        foreach ($loans_request as $loan) {
            ...
            if (in_array($loan->procedure_modality_id, $id_senasir)) {
                ...
                array_push($senasir_ancient, array(
                    ...
                    $loan->getBorrowers()->first()->registration_affiliate,
                    $loan->getBorrowers()->first()->registration_borrower,
                    $loan->getBorrowers()->first()->identity_card_borrower,
                    $loan->getBorrowers()->first()->city_exp_first_shortened_affiliate,
                    $loan->getBorrowers()->first()->last_name_borrower,
                    $loan->getBorrowers()->first()->mothers_last_name_borrower,
                    $loan->getBorrowers()->first()->surname_husband_borrower,
                    $loan->getBorrowers()->first()->first_name_borrower,
                    $loan->getBorrowers()->first()->second_name_borrower,
                    ...
                    $loan->guarantor_amortizing? $loan->borrowerGuarantors->first()->affiliate->identity_card : '',
                    $loan->guarantor_amortizing? $loan->borrowerGuarantors->first()->affiliate->registration : '',
                    $loan->guarantor_amortizing? $loan->borrowerGuarantors->first()->affiliate->full_name : '',
                    "***",
                    ...
                    $loan->guarantor_amortizing? $loan->borrowerGuarantors->first()->affiliate->city_identity_card->first_shortened : '',
    ...
    public function report_request_command_payments($period_id,$date){
        ...
        foreach ($current_loans as $loan) {
            ...
            array_push($data, array(
                ...
                $loan->interest->annual_interest,
                $loan->guarantor_amortizing ? 'Amort. Garante':'Amort. Titular',
                $loan->guarantor_amortizing ? $loan->borrowerGuarantors->first()->state_affiliate->name : '',
                $loan->guarantor_amortizing ? $loan->borrowerGuarantors->first()->state_affiliate->type_state_affiliate->name : '',
                $loan->guarantor_amortizing ? $loan->borrowerGuarantors->first()->registration : '',
                $loan->guarantor_amortizing ? $loan->borrowerGuarantors->first()->identity_card : '',
                ...
                $loan->guarantor_amortizing ? Util::money_format($loan->borrowerGuarantors->first()->get_amount_payment($estimated_date_calculate,false,'T')) : '',
                ...
                $loan->guarantor_amortizing ? ($loan->borrowerGuarantors->count() > 1 ? $loan->borrowerGuarantors->last()->state_affiliate->name : '') : '',
                ...
                $loan->guarantor_amortizing ? ($loan->borrowerGuarantors->count() > 1 ? Util::money_format($loan->borrowerGuarantors->last()->get_amount_payment($estimated_date_calculate,false,'T')) : '') : '',
            ));
        }    
         $export = new ArchivoPrimarioExport($data);
